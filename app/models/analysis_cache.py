...
class AnalysisCache(db.Model):
    """Cache for analysis results (group-based + category reference)."""
    ...
    # NEW: real-world grouped event reference
    group_id = db.Column(
        db.Integer,
        nullable=False,
        unique=True,
    ...
    __table_args__ = (
        ...
        db.CheckConstraint(
            'public_pct BETWEEN 0 AND 100',
            name='check_public_pct'
        ),
        db.CheckConstraint(
            'neutral_pct BETWEEN 0 AND 100',
            name='check_neutral_pct'
        ),
        db.CheckConstraint(
            'political_pct BETWEEN 0 AND 100',
            name='check_political_pct'
        ),
    )

    def __repr__(self):
        return f'<AnalysisCache group:{self.group_id}>'